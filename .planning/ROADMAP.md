...
- 🚧 **v5.0 Feature Parity & Differentiation** — Phases 18-23 (in progress)
- 🚧 **v6.0 Proactive Knowledge & Onboarding** — Phases 24-28 (in progress)
...
- [x] 18-02-PLAN.md — v5.0 schema columns, TypeScript types, prepared statements
...
- [x] 19-02-PLAN.md — Wire dedup into remember/recall + entity merge + integration tests
...
**Goal**: The database has a project_paths table that maps filesystem directories to project entities, enabling all downstream features to resolve a working directory to a scoped knowledge set
**Depends on**: Phase 23 (v5.0 complete) — or can run in parallel with v5.0 later phases since it is additive schema
...
**Success Criteria** (what must be TRUE):
  1. Starting a Claude Code session in a registered project directory causes relevant project knowledge to appear in the agent's context without any explicit `myco recall` call
  2. The injected context never exceeds 1,500 tokens — workflow rules appear before project facts, and the injection ends with "More context available — call `myco recall`" when the cap is reached
...
**Success Criteria** (what must be TRUE):
  1. Running `myco init` in a project directory reads README.md, CLAUDE.md, package.json, tsconfig.json, and git config, then presents a batch summary of inferred entities for approval before writing anything to the graph
...
**Depends on**: Phase 25 (rules must be surfaced before they are created; correction depends on temporal versioning from Phase 19)
**Requirements**: RULE-01, RULE-02, RULE-03, CORRECT-01, CORRECT-02
**Success Criteria** (what must be TRUE):
  1. Calling `remember_rule` with an instruction like "always update docs before committing" stores it as a `workflow_rule` entity with `decay_exempt=true` and `confidence=1.0`
...
**Depends on**: Phase 27 (entity type conventions), Phase 26 (project data to corroborate against)
...
| 19. Temporal Versioning + Dedup Resolution | v5.0 | 2/2 | Complete | 2026-03-27 |
