...
**Domain:** Local-first AI agent memory system — MCP server, knowledge graph, embeddings, PWA dashboard
**Researched:** 2026-03-20
**Confidence:** HIGH (versions verified via GitHub releases and npm)
...
```bash
# MCP Server core
npm install @modelcontextprotocol/sdk better-sqlite3 sqlite-vec ollama zod node-cron nanoid

# HTTP API layer (serving data to PWA)
npm install hono

# Dev dependencies — MCP server
npm install -D typescript tsx tsup @types/node @types/better-sqlite3 vitest

# PWA / Dashboard (separate workspace or directory)
npm install react react-dom @tanstack/react-query @tanstack/react-router hono date-fns react-force-graph-2d

# Dev dependencies — Dashboard
npm install -D vite vite-plugin-pwa tailwindcss @vitejs/plugin-react typescript
...
**If building MCP server only (no PWA yet):**
...
**If PWA needs to run offline (no local server):**
...
**If the consolidation LLM call needs to be swappable:**
...
**If knowledge graph queries get complex:**
...
---
*Stack research for: Local-first AI agent persistent memory — MCP server + knowledge graph + embeddings + PWA*
*Researched: 2026-03-20*
