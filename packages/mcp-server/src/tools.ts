...
export async function rememberEntity(
  ...
  if (classification.classification === 'NOOP') {
    ...
    // Phase 28: Still check for preference promotion even on NOOP
    // (same preference from a different project should trigger promotion)
    if (entity_type === 'user_preference' && project !== undefined) {
      ...
      if (promotion.promoted) {
        ...
        return {
          content: [{
            type: 'text' as const,
            text: `Observation already exists for "${entity_name}" — skipped (${classification.reason}). Promoted to global preference (sources: ${sources}).`,
          }],
        };
      }
    }
